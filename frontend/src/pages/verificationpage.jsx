...
//   const isLoading = false
  ...
  const{verificationEmail,error,isLoading} = useAuthStore()
  ...
  const handleChange = (index, value) => {
	...
	const handleSubmit = async (e) => {
		...
		try {
			await verificationEmail(verificationCode);
			navigate("/");
					...
					<div className='flex justify-between'>
						{code.map((digit, index) => (
							<input
								...
								className='w-12 h-12 text-center text-2xl font-bold  text-black border-2 border-gray-600 rounded-lg focus:border-green-500 focus:outline-none'
							/>
						))}
					</div>
